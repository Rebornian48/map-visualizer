...
function YearSelect({ label, value, onChange, years, busy }) {
  return (
    <div>
      ...
      <select value={value} onChange={(e) => onChange(Number(e.target.value))} style={fieldStyle} disabled={busy}>
...
function MonthSelect({ label, value, onChange, busy }) {
  return (
    <div>
      <div style={{ fontSize: '0.7rem', color: 'var(--text-dim)', marginBottom: 4 }}>{label}</div>
      <select value={value} onChange={(e) => onChange(Number(e.target.value))} style={fieldStyle} disabled={busy}>
...
export default function ExportModal({ yearData, map, onClose }) {
  ...
  const [startYear, setStartYear] = useState(years.at(0))
  const [endYear, setEndYear] = useState(years.at(-1))
...
function TitleField({ title, setTitle, busy, placeholder }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <label style={labelStyle}>Video title</label>
      <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
             disabled={busy} maxLength={80} placeholder={placeholder} style={fieldStyle} />
      <div style={{ fontSize: '0.7rem', color: 'var(--text-dim)', marginTop: 4, fontFamily: "'DM Mono', monospace" }}>
...
function ProgressBar({ stage, progress }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: '0.75rem', fontFamily: "'DM Mono', monospace", color: 'var(--text-dim)', marginBottom: 6 }}>{stage}</div>
      <div style={{ width: '100%', height: 4, background: 'var(--surface-2)', borderRadius: 2, overflow: 'hidden' }}>
        <div style={{ width: `${progress * 100}%`, height: '100%', background: 'var(--accent)', transition: 'width 0.15s' }} />
      </div>
    </div>
  )
}
...
function Actions({ busy, onClose, onGenerate, canGenerate }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
      ...
      <button onClick={onGenerate} disabled={!canGenerate} style={{
        ...
        borderRadius: 8, color: 'white', cursor: canGenerate ? 'pointer' : 'not-allowed',
